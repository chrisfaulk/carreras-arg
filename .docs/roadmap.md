...
> Mapa de Fase 1 para lookup rápido: `backend/src/shared/*` (tokens, paginación, throttling, filtros), `backend/src/modules/identity/*` (1.1–1.4), `backend/src/modules/catalog/*` (1.5–1.9, `correlative-graph.ts` puro), `backend/test/identity.test.mjs` + `catalog.test.mjs`, `backend/prisma/schema.prisma` (tablas hasta `subject_correlative` ya migradas). Fase 2 toca: `backend/src/modules/enrollment/*` (2.1: service+controller+module, UQ `user+plan`), `backend/src/modules/tracking/*` (2.2–2.6 y 2.10: service+controller+module más `availability-reader.ts`, `attempt-closer.ts` y `average-calculator.ts` puros sin I/O), `backend/src/modules/evaluation/*` (2.7–2.9: instances, retakes, final-exams con ownership via enrollment y `FOR UPDATE`), `backend/src/shared/read-models.ts` (nuevo: projections y ownership `enrollment.user_id`), `backend/test/tracking.test.mjs` + `evaluation.test.mjs` (caja negra: agregado, disponibilidad, máquina, cierre, mejor-nota, bloqueo de final, promedios x2), `frontend/app/dashboard/*` + `frontend/features/tracking/*` (primer UI real según `pages.md`: tabs por estado, banner `insufficientCorrelatives`).
